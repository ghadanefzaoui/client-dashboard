...
const Customer: React.FC<Props> = ({ name, orderCount }) => {
  ...
  return (
    <Link to={"/"} className={"flex items-center w-full py-1"} style={{border: "1px solid rgba(239, 68, 68, 1)"}}>
      {/* Avatar */}
      <div className="w-14 h-14 flex justify-center items-center font-bold text-xl text-grey bg-glitch-box rounded-full">
        {customerAvatar}
      </div>
      <div className="ml-5 flex justify-center flex-col">
        {/* Name */}
        <h2 className="font-bold leading-3 text-lg text-grey">
          {customerName}
        </h2>
        {/* Order count */}
